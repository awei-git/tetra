...
class SmartBackfiller:
    ...
    async def backfill_symbol(self, symbol: str, timeframe: str = "1d", days_back: int = 730) -> Dict[str, int]:
        ...
        end_date = date.today()
        if last_date:
            # If we have data, start from day after last update
            ...
            logger.info(f"{symbol}: No existing data, fetching {days_back} days from {start_date}")
    ...
    async def run_daily_backfill(self):
        ...
        for symbol in high_priority_symbols:
            ...
            # Get daily data
            stats = await self.backfill_symbol(symbol, "1d", days_back=730)  # 2 years
        ...
        for i, symbol in enumerate(remaining_symbols):
            ...
            stats = await self.backfill_symbol(symbol, "1d", days_back=365)  # 1 year
            total_stats["yfinance_records"] += stats["yfinance"]
            total_stats["polygon_records"] += stats["polygon"]
            total_stats["errors"] += stats["errors"]
            total_stats["symbols_processed"] += 1
...
async def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--scheduled":
        ...
        if choice == "1":
            ...
            for symbol in symbols:
                print(f"\nBackfilling {symbol}...")
                stats = await backfiller.backfill_symbol(symbol)
